...
public struct NumberPairing: Equatable, Comparable, Hashable {
    ...
    public var firstNumber: Double {
        get { storedNumber }
        set { storedNumber = validateAndCorrectNumberInput(of: newValue) }
    }
    
    public var secondNumber: Double {
        get { sumOfNumbers - storedNumber }
        set { storedNumber = sumOfNumbers - validateAndCorrectNumberInput(of: newValue) }
    }
    
    var product: Double { storedNumber * secondNumber }
    var difference: Double { abs(storedNumber - secondNumber) }
    ...
    public func difference(_ anotherNumberPairing: NumberPairing) -> Double { abs(result - anotherNumberPairing.result) }

    /// This will test if two results are close enough to be considered equal to each other
    /// The two NumberPairings may still be !=
    public func isEquivalentTo(_ anotherNumberPairing: NumberPairing) -> Bool { difference(anotherNumberPairing) < minimumPrecision }
    ...
    public func longReport() -> String {
        ...
        let differenceRounded = roundNumberToString(from: difference, withPrecision: 100_000)
    ...
    public func hash(into hasher: inout Hasher) {
        hasher.combine(storedNumber)
        hasher.combine(sumOfNumbers)
    }
